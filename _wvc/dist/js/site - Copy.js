...
$(".news-filters--toggle").click(function () {
  $(this).toggleClass('active');
  $('.filters-collapsed').slideToggle("slow");
});
$(".news-filters--grid").click(function () {
  $(".news-layout").addClass('news-layout--grid').removeClass('news-layout--list');
});
$(".news-filters--list").click(function () {
  $(".news-layout").addClass('news-layout--list').removeClass('news-layout--grid');
});
...
$('.tabsVertical-nav').each(function () {
  ...
  $(this).parents('.col-md-3').next('.col-md-9').find('.tab-content--vertical').css("min-height", navHeight);
});
...
$(window).load(function () {
  ...
  $('.interactive-image_button').click(function () {
    $(this).parent('.interactive-image_item').addClass('interactive-image_item-shown');
  });
  ...
  $('.interactive-image_item').each(function () {
    ...
    var percentLeft = position.left / $('.interactive-image-content').width() * 100;
    var percentTop = position.top / $('.interactive-image-content').height() * 100;
...
$('.wvc-ll-btn').click(function () {
  $(this).parent('.col-md-12').prev('.news-layout').find('.news-row:last a').focus();
});
$(window).on('load resize', function () {
  if ($(window).width() < 1201 - (window.innerWidth - document.documentElement.clientWidth)) {
    if (!$('#navbar').has('.siteHeader-navBarLeft_topbar').length) {
      $('<ul class="nav navbar-nav siteHeader-navBarLeft siteHeader-navBarLeft_topbar"></ul>').insertAfter("#navbar .siteHeader-search_mobile");
      $('.siteHeader-topItem').each(function () {
        $(this).clone().wrap('<li class="siteHeader-navItem siteHeader-navItem_topbar"></li>').parent().appendTo('.siteHeader-navBarLeft_topbar');
      });
    }
  } else {
...
if (window.location.href.indexOf("/region/") > -1) {
  ...
  filterTitle = $('.page-header--breadcrumbs').text().replace(parentText, "").trim();
  $('.stories-section--title strong').after("<p class='stories-section--subtitle'>Displaying stories with '<strong>" + filterTitle + "</strong>' region</p>");
  $(function () {
    $('.filters a').each(function () {
      if ($(this).attr("href") === window.location.pathname) {
        $(this).addClass("active");
...
if (window.location.href.indexOf("/category/") > -1) {
  parentText = $().add($('.page-header--breadcrumbs').html()).text();
  filterTitle = $('.page-header--breadcrumbs').text().replace(parentText, "").trim();
  $('.stories-section--title strong').after("<p class='stories-section--subtitle'>Displaying stories with '<strong>" + filterTitle + "</strong>' category</p>");
  $(function () {
...
if (window.location.href.indexOf("&tagname=") > -1) {
  var match = RegExp('[?&]' + 'tagname' + '=([^&]*)').exec(window.location.search);
  filterTitle = decodeURIComponent(match[1].replace(/\+/g, ' ')); //var filterTitle = getParameterByName('tagName').trim();

  $('.stories-section--title strong').after("<p class='stories-section--subtitle'>Displaying stories with '<strong>" + filterTitle + "</strong>' tag</p>");
  $(function () {
    $('.stories-filter a').each(function () {
      if ($(this).attr("href") === window.location.pathname) {
        $(this).addClass("active");
...
$(document).ready(function () {
  ...
  var url = dict_lang_store_url[$('html').attr('lang')]; //var url = $('html').attr('lang') === 'en-CA' ? url_en : url_fr;
  ...
  $.ajax({
    ...
    success: function success(e) {
      ...
      if (e.item_count > 0) {
        var badgeItemCount = '<span id="basket-basketIndicator" class="basket-basketIndicator">' + e.item_count + '</span>';
        $('.siteHeader-basket').append(badgeItemCount);
        $('.siteHeader-basket').attr('aria-label', l + '. ' + e.item_count + ' items in the cart');
      } else {
        $('.siteHeader-basket').attr('aria-label', l + '. ' + 'No item in the cart');
      }
    },
